...
#On s'intéresse aux profils de températures des capitales européennes
#====================================================================

library(FactoMineR)
temperature <- read.table("http://factominer.free.fr/livre/temperat.csv",header=TRUE, sep=";", dec=".", row.names=1)

#Raliser une ACP sur les capitales eurpéennes en choisissant judicieusement les varaibles actives
res <- PCA(temperature, ind.sup=24:35, quanti.sup=13:16, quali.sup=17,scale=TRUE)


#Présenter le premier plan factoriel en habillant les individus avec en fonction de la variables R?gion
plot.PCA(res, choix="ind", habillage=17)

#méthode du coude consulter ce lien pour certaines options https://www.statmethods.net/graphs/bar.html
...
#ou à l'aide du package factoextra qui requiert celui de munsell
library(factoextra)
...
#Centrer et réduire, un mot sur le deuxième terme (Que dire de Helsinki et Athènes)
